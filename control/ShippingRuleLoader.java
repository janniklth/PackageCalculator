 ...
 * list of {@link ShippingRule} objects.
 *
 * @see ShippingRule
 */
public class ShippingRuleLoader {
    ...
    public static List<ShippingRule> loadCustomShippingRules(String rules_json_path) throws ShippingRuleException {
        ...
        if (url == null) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_FILE_NOT_FOUND, rules_json_path),
                    null);
        }

        try (InputStream inputStream = ShippingRuleLoader.class.getClassLoader().getResourceAsStream(rules_json_path)) {
            if (inputStream == null) {
                throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_FILE_OPEN_ERROR,
                        rules_json_path), null);
            }
    ...
    private static void validateShippingRule(ShippingRule rule, String rules_json_path) throws ShippingRuleException {
        if (rule.getType() == null || rule.getType().isEmpty()) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "type",
                    rules_json_path), null);
        }
        if (rule.getMaxLength() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "maxLength",
                    rules_json_path), null);
        }
        if (rule.getMaxWidth() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "maxWidth",
                    rules_json_path), null);
        }
        if (rule.getMaxHeight() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "maxHeight",
                    rules_json_path), null);
        }
        if (rule.getMaxWeight() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "maxWeight",
                    rules_json_path), null);
        }
        if (rule.getCost() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "cost",
                    rules_json_path), null);
        }
        // maxGirth is optional, but if present, it should be a valid positive value
        if (rule.getMaxGirth() != null && rule.getMaxGirth() <= 0) {
            throw new ShippingRuleException(String.format(Constants.SHIPPING_RULES_INVALID_FIELD, "maxGirth",
                    rules_json_path), null);
        }
    }
